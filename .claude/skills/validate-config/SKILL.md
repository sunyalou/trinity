...
**If must-fix issues > 0**, create or update an issue:

```bash
# Check for existing open config validation issue
EXISTING=$(gh issue list --repo abilityai/trinity --state open --limit 50 --json number,title --jq '.[] | select(.title | startswith("Config Validation:")) | .number' | head -1)
