...
> **Status**: Implemented
> **Created**: 2025-12-22
> **Updated**: 2026-05-29 (#970: headless executor decoupled from leaked reader threads — finalize-snapshot on budget-exceeded drain + widened outer timeout margin)
> **Verified**: 2026-02-05
...
| Date | Changes |
|------|---------|
| 2026-05-29 | **Issue #970 (Phase 1) - Headless executor decoupled from leaked reader threads**: When a headless task spawns grandchildren (stdio MCP servers, hooks, subagents) that hold the stdout pipe FD open after Claude exits, the reader thread wedges in `readline()`; the drain's force-close fallback can't unblock it (the #728 `safe_close_pipes` ↔ TextIOWrapper-lock deadlock), so the orchestrator used to block until the full task timeout (~2h), saturating execution slots and producing false "timed out after 7200s" failures even though Claude finished in seconds. This PR makes the executor *tolerate* a leaked reader rather than block on it (the drain-side `_drain_bounded` return contract is documented in [execution-termination.md](execution-termination.md)). Changes in `headless_executor.py`: (1) new `HeadlessRunContext.drain_budget_exceeded: bool` field (default `False` keeps the clean-drain fast path zero-copy) — set `True` when `_drain_bounded` returns `"budget_exceeded"` or `"errored"`. (2) `_run_headless_subprocess` snapshots the captured stdout exception **before** draining (`pre_drain_stdout_exc = ctx.stdout_exc[0] if ctx.stdout_exc else None`) and re-raises only that pre-drain capture — a leaked reader appending a late `ValueError` from the force-closed pipe can no longer mask the #285 permission-mode fast-fail or raise a spurious late exception (D16). (3) `_finalize_headless_result` snapshots every field it reads on the budget-exceeded path — `raw_messages`/`response_parts`/`execution_log`/`verbose_output_lines` via `list(...)`, `metadata` via `model_copy(deep=True)` — onto a fresh context via the new `_snapshot_for_finalize()` helper, so a still-leaked reader mutating the live buffers can't tear a read; the rest of finalize (including `_attempt_empty_result_recovery`'s in-place mutations) then operates on the isolated copy (D19). `model_copy(deep=True)` can itself lose the race (`RuntimeError: dictionary changed size during iteration` if the reader sets a field mid-copy), so `_snapshot_for_finalize` retries up to `_SNAPSHOT_RETRY_ATTEMPTS` (3) and, if every attempt loses, logs a warning and falls back to the live `ctx` (no worse than pre-#970). The #678 in-memory-`response_parts`-over-JSONL precedence is preserved. (4) In `execute_headless_task`, the outer `asyncio.wait_for` safety-net margin widened from `effective_timeout + 60` to `effective_timeout + _DRAIN_BUDGET_SECONDS + 30` (D6) — the flat +60 was below the inner 90s drain budget, so the outer net could fire mid-drain and mislabel a bounded drain as an outer timeout; the +30 slack above the budget absorbs the pre-drain `_terminate_process_group(graceful_timeout=5)` and keeps the inner machinery strictly first-to-fire (~25s headroom). `_DRAIN_BUDGET_SECONDS` is now imported from `subprocess_lifecycle.py`. The **chat** path (`claude_code.py`) calls `_drain_bounded` fire-and-forget — outcome intentionally ignored; chat budget-exceeded recovery is a deferred follow-up (D18). Phase 2 (make `safe_close_pipes` abandonable + emit `outcome=hard_timeout`) is a follow-up ship. Tests: 36 new/updated unit tests — `tests/unit/test_headless_drain_budget.py`, `tests/unit/test_chat_drain_contract.py`, `tests/unit/test_drain_bounded.py`, `tests/unit/test_subprocess_pgroup.py`. |
