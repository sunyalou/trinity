...
6. Acquire `_ResumeLock(agent, cached_uuid, session_id, ttl_seconds=lock_ttl)` — Redis SET NX EX with async wait-and-retry (250 ms tick, 30 s ceiling). Warm-turn key: `session_lock:{agent}:{claude_session_id}` via `_session_lock_key(agent, uuid)`. Cold-turn key: `session_lock:cold:{session_id}` (#779 — previously cold turns short-circuited to no-op, letting two concurrent first POSTs race on `update_cached_claude_session_id` and orphan one JSONL inside the agent).
