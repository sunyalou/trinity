...
- `monitoring_service.py` - Fleet-wide health monitoring (MON-001). The authoritative ~30s fleet-health loop. **Lifespan-wired (#1121):** `main.py`'s `lifespan()` reads the persisted `system_settings.monitoring_config` on boot (staggered +12s) and starts the loop only when `enabled` is set — so the choice survives backend restarts (previously the loop was started *only* by `POST /api/monitoring/enable` and silently died on every restart). `MonitoringConfig.enabled` is the single source of truth and **defaults to OFF**; `enable`/`disable` and `PUT /config` persist it and reconcile the running loop. The `*_check_interval` fields reject non-positive values (Pydantic validator) and the loop clamps its sleep to ≥1s, so a bad config can't produce a tight check-flood. Checks the whole fleet → `agent_health_checks` (#675), pruned to a 7-day default by the cleanup-service retention sweep (#772).
- `monitoring_alerts.py` - Alert threshold configuration
- `heartbeat_service.py` - Agent push-heartbeat liveness layer (RELIABILITY-004, #307). Owns all Redis heartbeat keys; `record_heartbeat` (SETEX 15s + persistent `seen` marker), `read_heartbeat`, `heartbeat_status`/`heartbeat_status_bulk` (one pipelined round-trip, D4), `authorize_heartbeat` (Option B — only the agent's own agent-scoped MCP key), and `run_heartbeat_watch_loop`/`process_watch_tick` (5s loop, 3-miss guard, fires a cooldown-debounced operator alert via `monitoring_alerts` on the alive→stale transition; writes no health row). Additive to the lifespan-wired ~30s `monitoring_service.py` (default-off, #1121), which stays authoritative for aggregate status when enabled.
...
| **Monitoring Service** | `monitoring_service.py` | Authoritative ~30s fleet-wide health-check loop. **Lifespan-wired & restart-surviving (#1121):** resumed at boot from the persisted `monitoring_config` (single source of truth, `enabled` **defaults OFF**); started only when enabled, so it no longer dies silently on every backend restart. `enable`/`disable`/`PUT /config` persist the flag and reconcile the running loop; `*_check_interval` validated > 0 (loop also clamps sleep ≥1s). (MON-001, #1121) |
| **Heartbeat Watch Loop** | `heartbeat_service.py` | 5s loop (staggered +10s) that fires a soft, cooldown-debounced operator alert (via the existing `monitoring_alerts` notification path) after 3 consecutive missed agent push-heartbeats — and a recovery notification when beats resume. Reads `seen`-marked agents via a batched Redis pipeline; per-tick miss counters live in Redis. Alerts fire **only on the alive→stale transition** (and recovery only after a prior downgrade), so the operator gets one alert per loss episode. It writes **no** health-check row — the lifespan-wired ~30s monitoring loop (default-off, #1121) stays authoritative for aggregate status when enabled. Soft severity + 3-miss guard keep the silent-fail heartbeat's false positives recoverable. Old-image agents (no `seen` marker) resolve to `unsupported` and are ignored. (RELIABILITY-004, #307) |
