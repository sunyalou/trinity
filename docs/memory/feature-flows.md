...
|------|-----|---------|------|
| 2026-05-29 | #970 | fix(agent-runtime): headless executor decoupled from leaked reader threads (Phase 1). `_drain_bounded` now returns a `DrainOutcome` literal (`completed`/`budget_exceeded`/`errored`) instead of `None` and captures+logs daemon-thread exceptions (was a silent `except Exception: pass`). Headless path records `HeadlessRunContext.drain_budget_exceeded`, snapshots the pre-drain `stdout_exc` (preserves the #285 permission-mode fast-fail), and `_finalize_headless_result` deep-copies every read field via `_snapshot_for_finalize` (retry-guarded) on the budget-exceeded path so a still-leaked reader can't tear a read. Outer `asyncio.wait_for` margin widened to `+_DRAIN_BUDGET_SECONDS+30` so the safety net can't fire mid-drain. Chat path drains fire-and-forget (recovery deferred). Stops the false "timed out after 7200s" slot-saturation when Claude actually finished in seconds. 36 unit tests. | [execution-termination.md](feature-flows/execution-termination.md), [parallel-headless-execution.md](feature-flows/parallel-headless-execution.md) |
