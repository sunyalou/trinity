...
| 2026-04-22 | SCHED-COND-001 (#454) | Conditional schedule pre-check — agent-owned `POST /api/pre-check` hook lets templates deterministically skip scheduled ticks (zero Claude tokens on empty polls); fail-open; reuses `ExecutionStatus.SKIPPED` (no schema change) | [scheduler-pre-check.md](feature-flows/scheduler-pre-check.md) |
