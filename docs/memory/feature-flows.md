...
|------|-----|---------|------|
| 2026-06-19 | #1116 | feat(ui): in-app bug reporting from the floating Help widget. New **"Report a bug"** tab in `HelpChatWidget.vue` (no second button): form → **see-before-send review** (scrubbed preview + public-repo warning) → success w/ issue link. Diagnostics auto-captured (version/commit via `useBuildInfo`, route/url, UA/viewport/OS, recent console via a new `utils/consoleBuffer.js` ring buffer installed at bootstrap) and **scrubbed client-side** (`utils/scrub.js`) so the review shows exactly what's sent. Returned `issue_url` link-rendered only if `https://github.com/` (href guard). Build-time knobs `VITE_BUG_INTAKE_URL` / `VITE_BUG_REPORTING_ENABLED`. `intake.abilityai.dev` added to CSP `connect-src` in **both** `vite.config.js` + `security-headers.conf`. Credential-less by design: reports POST to a hosted Cloudflare Worker intake (`trinity-ops-agent/bug-intake/`, out-of-repo) on the stable `https://intake.abilityai.dev/v1/report-bug` domain, which holds the GitHub token server-side, **re-scrubs**, soft per-install caps, content-hash dedupes, and files a `type-bug`+`source:in-app` issue. Worker scrubber: 14 unit tests; verified live end-to-end against `abilityai/trinity`. Screenshot deferred. | [in-app-bug-reporting.md](feature-flows/in-app-bug-reporting.md) |
