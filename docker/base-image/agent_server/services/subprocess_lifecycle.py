...
# (#970). ``budget_exceeded`` / ``errored`` mean a reader thread may still be
# mutating ``ctx`` concurrently; the headless path snapshots its finalize-read
# fields on those outcomes.
...
def _drain_bounded(
    ...
    *threads: Optional[threading.Thread],
    ...
    Issue #970: returns the drain outcome instead of ``None`` so the headless
    orchestrator can detect the budget-exceeded / errored cases — both leave a
    leaked reader thread that may still be mutating the shared run context, so
    the caller snapshots its finalize-read fields before reading them:
    ...
    def _target() -> None:
        try:
            ...
            # #970: capture + log instead of swallowing. A drain that raises
            # used to be indistinguishable from a clean completion.
            errored.set()
            logger.exception(
